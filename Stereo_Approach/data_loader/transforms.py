...
def image_transforms_ours(mode='train', augment_parameters=[0.8, 1.2, 0.5, 2.0, 0.8, 1.2],
...
def image_transforms_cityscapes(mode='train', augment_parameters=[0.8, 1.2, 0.5, 2.0, 0.8, 1.2],
                                do_augmentation=True, transformations=None,  size=(256, 512)):
    if mode == 'train':
        data_transform = transforms.Compose([
...
class DoVal(object):
    def __call__(self, sample):
        ...
        sample = {'left_image': left_image, 'right_image': right_image, 'flipped_left_image': flipped_left}
        return sample
