...
def save_sample_images(x, g_right, gt_right, global_step, checkpoint_dir):
    x = (x.detach().cpu().numpy().transpose(0, 2, 3, 4, 1) * 255.).astype(np.uint8)
    g_right = (g_right.detach().cpu().numpy().transpose(0, 2, 3, 4, 1) * 255.).astype(np.uint8)
    gt_right = (gt_right.detach().cpu().numpy().transpose(0, 2, 3, 4, 1) * 255.).astype(np.uint8)

    refs, inps = x[..., 3:], x[..., :3]
    folder = join(checkpoint_dir, "samples_step{:09d}".format(global_step))
    if not os.path.exists(folder): os.mkdir(folder)
    collage = np.concatenate((refs, inps, g_right, gt_right), axis=-2)
...
def get_sync_loss(mel, g_right):
    g_right = g_right[:, :, :, g.size(3)//2:]
    g_right = torch.cat([g_right[:, :, i] for i in range(syncnet_T)], dim=1)
    # B, 3 * T, H//2, W
    a, v = syncnet(mel, g_right)
    y = torch.ones(g_right.size(0), 1).float().to(device)
...
def validate_cycle(global_epoch): # Modified
...
def train(args, device, model_Wav2Lip, train_data_loader_Right, test_data_loader_Right, optimizer, checkpoint_dir=None, checkpoint_interval=None, nepochs=None):
    ...
    while global_epoch < nepochs:
        ...
        prog_bar = tqdm(enumerate(train_data_loader_Right))
        
        for step, (x, indiv_mels, mel, gt_right) in prog_bar: # M
            model_Wav2Lip.train()
            optimizer.zero_grad()
            ...
            gt_right = gt_right.to(device) # Modified

            g_right = model_Wav2Lip(indiv_mels, x) # Modified

            if hparams.syncnet_wt > 0.:
                sync_loss = get_sync_loss(mel, g_right)
            else:
                sync_loss = 0.

            l1loss = recon_loss(g_right, gt_right) + (best_val_loss * recon_loss(g_right, gt_right)) # Modifieed
            ...
            if global_step % checkpoint_interval == 0:
                save_sample_images(x, g_right, gt_right, global_step, checkpoint_dir)
            ...
            if global_step == 1 or global_step % hparams.eval_interval == 0:
                with torch.no_grad():
                    average_sync_loss = eval_model(test_data_loader_Right, global_step, device, model_Wav2Lip, checkpoint_dir, best_val_loss)
...
def eval_model(test_data_loader_Right, global_step, device, model_Wav2Lip, checkpoint_dir, best_val_loss):
    eval_steps = 700
    print('Evaluating for {} steps'.format(eval_steps))
    sync_losses, recon_losses = [], []
    step = 0
    while 1:
        for x, indiv_mels, mel, gt_right in test_data_loader:
            step += 1
            model_Wav2Lip.eval()

            # Move data to CUDA device
            x = x.to(device)
            gt_right = gt_right.to(device)
            indiv_mels = indiv_mels.to(device)
            mel = mel.to(device)

            g_right = model_Wav2Lip(indiv_mels, x)

            sync_loss = get_sync_loss(mel, g_right)
            
            l1loss = recon_loss(g_right, gt_right) + (best_val_loss * recon_loss(g_right, gt_right))
...
if __name__ == '__main__':
    ...
    train_dataset_right = Dataset('train') # Right stereo image training dataset
    test_dataset_right = Dataset('val') # Right Stereo image validation dataset

    train_data_loader_Right = data_utils.DataLoader(train_dataset_right, batch_size=hparams.batch_size, shuffle=True,
        num_workers=hparams.num_workers)

    test_data_loader_Right = data_utils.DataLoader(test_dataset_right, batch_size=hparams.batch_size,num_workers=4)
    ...
    # Train!
    if args.mode == 'train':
        train(args, device, model_Wav2Lip, train_data_loader_Right, test_data_loader_Right, optimizer, checkpoint_dir=checkpoint_dir, checkpoint_interval=hparams.checkpoint_interval, nepochs=hparams.nepochs)
    elif args.mode == 'test':
