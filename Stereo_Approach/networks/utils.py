...
from .generator_resnet import ResNet50Pilzer
...
def define_generator_loss(args):
    if args.generator_loss == 'monodepth':
        loss, _ = MonodepthLoss(args) # Modified 
    else:
        raise NotImplementedError('Generator loss {} is not implemented'.format(args.generator_loss))
    return loss

# Function Added --
def define_generator_synth(args):
    if args.generator_loss == 'monodepth':
        loss, generator_synth = MonodepthLoss(args)
    else:
        raise NotImplementedError('Generator Synth {} is not implemented'.format(args.generator_loss))
    return generator_synth
